...
// UPDATED: Segment order now matches the anatomical sequence of the other components.
...
interface BarInfo { value: number; color: RiskColorVar | string; label: string; }
...
const ChartDisplay: React.FC<{
    ...
    const getBarInfo = (segment: Segment | undefined): BarInfo => {
        const noDataInfo: BarInfo = { value: 0, color: '--risk-dark-green', label: "0" };
        ...
        switch (configKey) {
            ...
            case 'FFRct': const ffrct = segment.ffrct ?? 0; return { value: ffrct, color: getFfrctColor(segment.ffrct), label: segment.ffrct ? formatNumber(ffrct, 2) : 'N/A' };
            ...
            case 'TPV': return { value: totalPlaque, color: getTpvColor(totalPlaque), label: formatNumber(totalPlaque, 1) };
            case 'PAV': return { value: segment.pav_pct, color: getPavColor(segment.pav_pct), label: formatNumber(segment.pav_pct, 1) };
                        ...
                        {allSegments.map((segmentData, index) => {
                            ...
                            return (
                                <div key={segId} className={styles.barWrapper}>
                                    ...
                                    <div className={styles.bar} style={{ height: `${visualHeight}%`, backgroundColor: `var(${color})` }}></div>
                                </div>
                            );
                        })}
                    </div>
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.controls}>
                {(["Stenosis", "FFRct", "Composition"] as MapMode[]).map(m => (
